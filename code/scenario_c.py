...
class MoveitRunner():
    def __init__(self, group_names, node_name='ariac_moveit_example',
        ...
        # Wait for the break_beam_1_change topic to become available
        rospy.loginfo("Waiting for breakbeam topics...")
        rospy.wait_for_message('/ariac/break_beam_1_change', Proximity)
        # Subscribe to the break_beam_1_change topic to get the image width and height
        rospy.Subscriber('/ariac/break_beam_1_change', Proximity, self.get_breakbeam1_info, queue_size = 1, buff_size = 1024)
        # And 3
        rospy.wait_for_message('/ariac/break_beam_3_change', Proximity)
        # Subscribe to the break_beam_1_change topic to get the image width and height
        rospy.Subscriber('/ariac/break_beam_3_change', Proximity, self.get_breakbeam3_info, queue_size = 1, buff_size = 1024)
        # And 4
        rospy.wait_for_message('/ariac/break_beam_4_change', Proximity)
        # Subscribe to the break_beam_1_change topic to get the image width and height
        rospy.Subscriber('/ariac/break_beam_4_change', Proximity, self.get_breakbeam4_info, queue_size = 1, buff_size = 1024)
    ...
    def define_preset_locations(self):
        ...
        # Below 3 locations are for the "moving obstacle" avoidance code
        name = 's11topright'
        gantry = [-11.35, 5.5, 0]
        left = None
        right = None
    ...
    def move_part(self, part, target, part_location, part_waypoint, agv):
        ...
        if 'shelf' in part_location:
            path = [pick_pose, near_pick_pose]
            self.cartesian_move(group, path)
            print("cartesian move done")
            self.goto_preset_location(part_location)
            print("part_location move done")
        if part_waypoint:
            self.goto_preset_location(part_waypoint)
        self.goto_preset_location('standby')
        ...
        print("is object attached: ", gm.is_object_attached())
    ...
    def move_part_moving_obstacle(self, part, target, part_location, part_waypoint, agv):
        # This example only uses the left arm
        group = self.groups['Left_Arm']
        gm = GripperManager(ns='/ariac/gantry/left_arm/gripper/')
        ...
        # We know something about the environment, so first let's move to the holding area between shelves 10 and 11
        # First, move to the right waypoint with the arm in the correct orientation
        self.goto_preset_location(part_waypoint)
        self.goto_preset_location('s11topright')
        self.goto_preset_location('s11topmiddle')

        num_attempts = 0

        while not (self.bb3_true > self.bb4_true and self.bb4_true > self.bb1_true):
            num_attempts += 1
            rospy.sleep(0.5)

        self.goto_preset_location('s11topleft')
        self.goto_preset_location(part_location)
        gm.activate_gripper()
    ...
    def get_breakbeam1_info(self, msg):
        if msg.object_detected == True:
            self.bb1_true = msg.header.stamp.secs
        else: 
            return

    def get_breakbeam3_info(self, msg):
        if msg.object_detected == True:
            self.bb3_true = msg.header.stamp.secs
        else: 
            return

    def get_breakbeam4_info(self, msg):
        if msg.object_detected == True:
            self.bb4_true = msg.header.stamp.secs
        else: 
            return
...
if __name__ == '__main__':
    ...
    for shipment in order.shipments:
        ...
        while True:
            ...
            print("len valid products: ", len(valid_products), ", actual products: ", valid_products)
            print("len candidate moves: ", len(candidate_moves))
            print("len all known parts: ", len(all_known_parts))
            ...
            if candidate_moves:
                ...
                if part_waypoint and (part_waypoint == "waypoints8right" or part_waypoint == "waypoints11left"):
                    move_successful = moveit_runner.move_part_moving_obstacle(
                        part,
                        world_target,
                        part_location,
                        part_waypoint,
